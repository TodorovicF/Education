...
int main () {
   Polynomial A(2,5,7), B(2,3,4), C(1,2), D(A), X, Y;
   Polynomial A2, B2, Z1, Z2;
   ...
   //     << "then exponent for each term. Enter -0 to terminate." << endl;
   ...
   int coeff = D.get_coeff(0); 
   cout << "coeff of subscript 0 of D is: " << coeff << endl;
   cout << "Doing getCoeff for term -20000" << endl;
   coeff = D.get_coeff(-20000);              // handle value out of range
   D.set_coeff(50,0);                        // set x^0 coeff to 50
   D.set_coeff(50,20000);                    // set x^20000 coeff to 50
   cout << "D =" << D << endl << endl;
